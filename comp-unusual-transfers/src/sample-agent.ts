...
import {
  ...
  COMPTROLLER_ABI,
  DISTRIBUTED_SUPPLIER_COMP_EVENT_SIG,
  DISTRIBUTED_BORROWER_COMP_EVENT_SIG,
  ERC20_TRANSFER_EVENT_SIG,
} from "./constants";

const web3 = new Web3(getJsonRpcUrl());
const compoundComptrollerContract = new web3.eth.Contract(
  COMPTROLLER_ABI as AbiItem[],
  COMPOUND_COMPTROLLER_ADDRESS
);
...
async function handleTransaction(txEvent: TransactionEvent) {
  ...
  const compDistributedEvents = txEvent
    .filterEvent(
      DISTRIBUTED_SUPPLIER_COMP_EVENT_SIG,
      COMPOUND_COMPTROLLER_ADDRESS
    )
  ...
  const [transferCompEvent] = txEvent.filterEvent(
    ERC20_TRANSFER_EVENT_SIG,
    COMPOUND_TOKEN_ADDRESS
  );
  ...
  const prevBlockCompAccrued = await compoundComptrollerContract.methods
    .compAccrued(txEvent.from)
    .call({}, blockNumber - 1);

  // calculate ratio of accrued to distributed COMP
  const accruedToDistributedRatio =
    amountCompDistributed.dividedBy(prevBlockCompAccrued);
  if (accruedToDistributedRatio.isGreaterThan(2)) {
    findings.push(
      Finding.fromObject({
        name: "Unusual COMP Distribution",
        description: `Distributed ${accruedToDistributedRatio.toFixed(
          0
        )}x more COMP to ${txEvent.from} than expected`,
        alertId: "COMP-1",
        protocol: "compound",
